...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function init(){
		if('<s:property value="adContentPush.apPushType" />' != ''){
	...
	<body class="content_bj" onload="init()">
		<form id="searchFrm" name="searchFrm"
			action="<%=basePath%>adContentPushManage/adContentPushManage!sendAdContent.action"
			method="post">
			<input type="hidden" name="apId"
				value='<s:property value="adContentPush.apId" />' />
			<div class="taole_h7">
						...
						<td valign="top" class="table_b_b">
							...
							<table class="add_table" border="0" cellspacing="0"
								...
								<tr class="add_table_tr"
									...
									<td class="add_table_right">
										<textarea id="apContent" name="apContent" class="add_textarea" style="resize: none;" rows="10" cols="80" dataType="Require" msg="请输入短信内容"><s:property value="adContentPush.apContent"></s:property></textarea>
									</td>
								</tr>
								<tr style="display: none" class="add_table_tr"
									onMouseOver="this.className='add_table_tr_over'"
									onMouseOut="this.className='add_table_tr'">
									<td class="add_table_left">
										填写时间:
									...
									<td class="add_table_right">
										<s:date name="adContentPush.apWriteTime"
								...
								<tr>
									<td colspan="2" class="add_btn">
										<input type="submit" class="btn1" value="发送"
											onclick="return confirm('确定要发送吗？')" />
										&nbsp;&nbsp;
										<input type="button" class="btn2" value="历史记录"
											onclick="showList()" />
									</td>
								</tr>
							</table>
